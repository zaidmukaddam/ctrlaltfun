...
export default function Home() {
  ...
  const [link, setLink] = React.useState<string>("https://ctrlalt.fun");
  ...
  React.useEffect(() => {
    if (redirect) {
      ...
      fetch("/api/upload", {
        ...
        .then((r) => r.json())
        .catch((err) => {
          toast("Could not upload coz" + err, {
            icon: "❌",
            id: loading,
          });
        })
      ...
      <main className="bg-gray-200 absolute w-full h-full flex flex-col items-center justify-center space-y-1">
        ...
        <header className="text-center justify-center content-center p-2">
          <motion.h3
            ...
            className="font-semibold text-3xl text-center p-4"
          >
            Hello, ctrl + alt + fun!
          </motion.h3>
          <motion.p
          ...
          <form className="flex flex-col space-y-3">
            ...
            <div className="flex items-center">
              <input
                ...
                className="form-checkbox focus:ring-0 transition-colors duration-150 h-4 w-4 appearance-none checked:text-black hover:checked:text-gray-900 checked:border-transparent border border-gray-300 rounded"
              />
              <label
                htmlFor="emojis"
